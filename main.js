...
const fogColor = new THREE.Color(0xbbf8fa); 
...
const animalSound = new THREE.Audio(listener);
audioLoader.load(
    'animal.mp3', 
    function(buffer) {
        animalSound.setBuffer(buffer);
        animalSound.setLoop(true);
        animalSound.setVolume(0.0); // Start completely silent
        animalSound.play(); 
    },
    undefined,
    function(err) { console.error("ERROR: Could not find 'animal.mp3'"); }
);
...
function createSandTexture() {
    ...
    //change color 3 color options max
    const c1 = new THREE.Color('#e8d293'); 
    const c2 = new THREE.Color('#e7ce89'); 
    const c3 = new THREE.Color('#e8d293'); 
...
// 4. Military Bases and drones
const bases = []; 
const allDrones = []; 

const droneGeo = new THREE.ConeGeometry(2, 2, 2); 
droneGeo.rotateX(Math.PI / 2); 
const droneMat = new THREE.MeshBasicMaterial({ color: 0x635b01 }); 
...
if (titleScreen) {
    titleScreen.addEventListener('click', () => {
        ...
        setTimeout(() => { isActive = true; }, 100); // Unlock the movement and camera
    });
}
...
function animate() {
    ...
    if (swarmCycle < previousCycle) {
        allDrones.forEach(d => {
            if (d.userData.isStalker) return; 
    ...
    if (isActive === true) {
        ...
        // Base sound
        // 1. Find distance to closest base
        ...
        for (let i = 0; i < bases.length; i++) {
            ...
            // Calculate how far viewer is from base center 
            const dist = Math.hypot(camera.position.x - baseX, camera.position.z - baseZ);
            if (dist < minDistanceToBase) {
                minDistanceToBase = dist;
            }
        }

        // 2. Map the distance to a volume level
        // If closer than 100 units, volume is 0. 
        // As you walk from 100 to 350 units away, volume fades up to 1.0.
        let targetVolume = THREE.MathUtils.mapLinear(minDistanceToBase, 100, 350, 0.0, 1.0);
        
        // Clamp it so it never goes below 0 or above 1.0
        targetVolume = THREE.MathUtils.clamp(targetVolume, 0.0, 1.0);
        
        // Apply the volume dynamically every frame
        if (animalSound.isPlaying) {
            animalSound.setVolume(targetVolume);
        }
